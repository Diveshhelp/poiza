...
class Photos extends Component
{
    ...
    protected $rules = [
        ...
        'mediaFiles.*' => 'required|image|mimes:jpeg,png,jpg,gif,webp',
    ];

    public function loadTodo($uuid)
    {
        try {
            $todo = Photo::where('uuid', $uuid)->firstOrFail();
            $this->todoUuid = $uuid;
    ...
    {
        $attachment = PhotoAttachment::findOrFail($attachmentId);
        // Delete the file from storage
        Storage::disk('public')->delete($attachment->file_path);
        // Delete the record
        $attachment->delete();
        // Refresh the existing files list
        $this->existingMediaFiles = PhotoAttachment::where('photo_id', $attachment->photo_id)->get();
    }
    ...
    public function savePhoto()
    {
        ...
        try {
            if ($this->isUpdate) {
                $todo = Photo::where('uuid', $this->todoUuid)->firstOrFail();
                $todo->update([
            ...
            if (!empty($this->mediaFiles)) {
                foreach ($this->mediaFiles as $file) {
                    ...
                    $filePath = $file->store('photo-attachments/'.Auth::id().'/'.$todo->id, 'public');
    ...
    {
        return view('livewire.photos.photo-add')->layout('layouts.app');
    }
}