...
x-data="{ 
    open: false,
    selectAll: false, // Start with false
    toggleAll() {
...
x-show="open"
x-init="
    window.addEventListener('open-bulk-email-modal', () => { 
        open = true;
        // Force selection to true when opening
        selectAll = true;
        $nextTick(() => $wire.toggleAllAttachments(true));
    });
    window.addEventListener('open-bulk-email-modal', () => { 
        open = true;
        // Force selection to true when opening
    ...
    window.addEventListener('close-bulk-email-modal', () => { open = false });
"
...
>
    ...
    >
        ...
        <form wire:submit.prevent="sendBulkAttachments">
            <div class="p-6">
                ...
                <div class="mb-4">
                    ...
                    <div class="relative">
                        ...
                        <!-- Loading indicator -->
                        <div wire:loading wire:target="search" class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                            ...
                            <div class="absolute z-10 w-full mt-1 bg-white rounded-md shadow-lg border border-gray-200">
                                <ul class="py-1 max-h-60 overflow-auto">
                                    @foreach ($results as $email)
                                        <li 
                                            wire:key="email-{{ $loop->index }}"
                                            wire:click="selectEmail('{{ $email }}')"
                    ...
                    <input type="hidden" wire:model="emailAddress">
