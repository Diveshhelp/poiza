<div class="p-6 mx-auto">
    ...
    @if (session()->has('message'))
        <div class="mb-4 p-4 bg-emerald-100 border border-emerald-200 text-emerald-800 rounded-xl shadow-sm">
            {{ session('message') }}
        </div>
    @endif
    ...
    @if($isOpen)
        <div wire:ignore.self class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 backdrop-blur-sm p-4">
            <div class="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-3xl p-6 max-h-[90vh] overflow-y-auto shadow-2xl border border-gray-100 dark:border-gray-700">
                ...
                <form wire:submit.prevent="store">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input wire:model="order_number" type="text" readonly class="mt-1 block w-full border border-gray-300 bg-gray-100 dark:bg-gray-700 dark:border-gray-600 rounded-md p-2 text-gray-600 dark:text-gray-300 cursor-not-allowed">
                        ...
                        <div>
                            ...
                            <input wire:model="customer_name" type="text" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md p-2 dark:bg-gray-700 dark:text-white">
                            @error('customer_name') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Customer Phone</label>
                            <input wire:model="customer_phone" type="text" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md p-2 dark:bg-gray-700 dark:text-white">
                        ...
                        <div>
                            ...
                            <input wire:model="customer_email" type="email" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md p-2 dark:bg-gray-700 dark:text-white">
                            @error('customer_email') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>

                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Shipping Address</label>
                            <textarea wire:model="shipping_address" rows="2" class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md p-2 dark:bg-gray-700 dark:text-white"></textarea>
                    ...
                    <div class="mt-6">
                        <div class="flex justify-between items-center mb-2">
                            <h4 class="text-sm font-bold text-gray-700 dark:text-gray-300 uppercase">Order Items</h4>
                        ...
                        <div class="space-y-3">
                            @foreach($orderItems as $index => $item)
                                <div wire:key="order-item-row-{{ $index }}" class="flex flex-col sm:flex-row items-center gap-3 bg-gray-50 dark:bg-gray-700/30 p-3 rounded-xl border border-gray-200 dark:border-gray-700">
                                    <div class="flex-1 w-full">
                                        <select wire:model.live="orderItems.{{ $index }}.my_product_id" class="w-full border border-gray-300 dark:border-gray-600 rounded-md p-2 text-sm dark:bg-gray-700 dark:text-white">
                                    ...
                                    <div class="w-full sm:w-28">
                                        <select wire:model.live="orderItems.{{ $index }}.unit_type" class="w-full border border-gray-300 dark:border-gray-600 rounded-md p-2 text-sm dark:bg-gray-700 dark:text-white">
                                            <option value="piece">Piece</option>
                                            <option value="box">Box</option>
                                        </select>
                                    </div>
                                    <div class="w-full sm:w-24">
                                        <input wire:model.live="orderItems.{{ $index }}.quantity" type="number" min="1" placeholder="Qty" class="w-full border border-gray-300 dark:border-gray-600 rounded-md p-2 text-sm dark:bg-gray-700 dark:text-white text-center">
                                    </div>
                                    <div class="w-full sm:w-28">
                                        <input wire:model="orderItems.{{ $index }}.price" type="number" step="0.01" placeholder="Price" class="w-full border border-gray-300 dark:border-gray-600 rounded-md p-2 text-sm dark:bg-gray-700 dark:text-white">
                                    </div>
                                    <div>
                                        <button type="button" wire:click="removeItemRow({{ $index }})" class="text-red-500 hover:text-red-700 p-2 font-bold text-lg">&times;</button>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="mt-6 flex justify-end space-x-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                        <button type="button" wire:click="closeModal" class="bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-2 rounded-xl text-sm font-medium transition">Cancel</button>
                        <button type="submit" wire:loading.attr="disabled" wire:target="store" class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 rounded-xl text-sm font-medium shadow transition flex items-center gap-2">
    ...
    @if($isViewModalOpen && $viewingOrder)
        <div wire:ignore.self class="fixed inset-0 z-50 overflow-y-auto bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center p-4">
            <div class="bg-white dark:bg-gray-800 rounded-2xl max-w-3xl w-full p-6 shadow-2xl border border-gray-200 dark:border-gray-700 max-h-[92vh] overflow-y-auto">
                ...
                <div class="space-y-5 text-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 dark:bg-gray-700/30 p-3.5 rounded-xl border border-gray-100 dark:border-gray-700">
                        <div>
                            <span class="text-xs text-gray-400 dark:text-gray-400 block font-bold uppercase tracking-wider mb-1">Customer Details</span>
                        ...
                        <div>
                            <span class="text-xs text-gray-400 dark:text-gray-400 block font-bold uppercase tracking-wider mb-1">Shipping Address</span>
                    ...
                    <div class="flex flex-wrap justify-between items-center bg-gray-100 dark:bg-gray-700/50 p-3.5 rounded-xl gap-3 border border-gray-200 dark:border-gray-600">
                        <div>
                        ...
                        <div>
                            ...
                            <span class="inline-block px-2.5 py-1 text-xs font-bold uppercase rounded-lg bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300">
                    ...
                    <div>
                        ...
                        <div class="border rounded-xl overflow-hidden dark:border-gray-700 shadow-sm">
                    ...
                    <div class="flex justify-end pt-1">
                        <div class="w-60 space-y-1.5 text-xs bg-gray-50 dark:bg-gray-700/30 p-3 rounded-xl border border-gray-100 dark:border-gray-700">
