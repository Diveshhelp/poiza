<div class="py-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 pb-4 border-b border-gray-200 gap-4">
        ...
        <a href="{{ url()->previous() }}" class="text-xs bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-2 rounded-lg font-semibold transition self-start md:self-auto">Back</a>
    </div>
...
@endif
    ...
    @if($activeStep === 'turning')
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <div class="mb-4">
                <h3 class="text-base font-bold text-gray-900">Turning Department Tracking</h3>
                <p class="text-xs text-gray-500">Update processed turning quantities against target orders.</p>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    <thead class="bg-gray-50 text-gray-500 uppercase text-xs">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left">Model Key</th>
                            <th class="px-4 py-3 text-center">Ordered Qty</th>
                            <th class="px-4 py-3 text-center">Turning Completed Qty</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        @foreach($order->items as $item)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-xs text-gray-600">{{ $item->model_key ?? 'N/A' }}</td>
                                <td class="px-4 py-3 text-center text-gray-500 font-semibold">{{ $item->quantity }} {{ $item->unit_type }}</td>
                                <td class="px-4 py-3 text-center">
                                    <input type="number" wire:model="quantities.{{ $item->id }}.turning_qty" class="w-28 text-center rounded-lg border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
    ...
    @if($activeStep === 'buff')
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <div class="mb-4">
                <h3 class="text-base font-bold text-gray-900">Buff Department Tracking</h3>
                <p class="text-xs text-gray-500">Update polished/buffed quantities prior to dispatch.</p>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    <thead class="bg-gray-50 text-gray-500 uppercase text-xs">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left">Model Key</th>
                            <th class="px-4 py-3 text-center">Ordered Qty</th>
                            <th class="px-4 py-3 text-center">Buff Completed Qty</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        @foreach($order->items as $item)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-center text-gray-500 font-semibold">{{ $item->quantity }} {{ $item->unit_type }}</td>
                                <td class="px-4 py-3 text-center">
                                    <input type="number" wire:model="quantities.{{ $item->id }}.buff_qty" class="w-28 text-center rounded-lg border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
            ...
            <!-- Save Button with Loader -->
            <div class="mt-6 flex justify-end">
                <button wire:click="updateBuff" wire:loading.attr="disabled" class="inline-flex items-center px-5 py-2 bg-indigo-600 text-white rounded-lg text-xs font-semibold hover:bg-indigo-700 transition disabled:opacity-50">
                    ...
                    <span wire:loading.remove wire:target="updateBuff">Save Buff Quantities</span>
