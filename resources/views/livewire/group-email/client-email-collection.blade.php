    ...
    <div class="mx-auto py-5 sm:px-6 lg:px-8">
        ...
        <div class="md:flex md:items-center md:justify-between">
            <div class="min-w-0 flex-1">
                <h3 class="text-lg font-medium text-gray-900">Client Email Manager</h3>
                <p class="mt-2 text-sm text-gray-600 leading-relaxed">
                    Send formatted emails to your clients
                </p>
            </div>
        </div>
        ...
        <div class="mt-4">
            ...
            <!-- Compose Email Tab with Side-by-Side Preview -->
                            ...
                            <div class="sticky top-4">
                                <div class="border border-gray-300 rounded-lg bg-gray-50 h-full">
                                    ...
                                    <div class="rounded-md border border-gray-200 m-3 bg-white overflow-hidden">
                                        ...
                                        <!-- Email Body -->
                                        <div class="p-4 text-sm text-gray-800 overflow-y-auto" style="max-height: auto;">
                                            <div wire:ignore id="previewContent" class="prose prose-sm max-w-none">
                                                <div class="content">
                                                {!! $emailContent ? nl2br(e($emailContent)) : '<span class="text-gray-400">Email body preview will appear here as you type...</span>' !!}
                                                </div>
                                            </div>
                                            
                                            <div class="footer mt-15">
                                                <p>© {{ date('Y') }} Powered By {{ env('APP_NAME') }}. All rights reserved.</p>
                                            </div>
                                        </div>
                                    </div>
            ...
            <!-- Customers Tab -->
            <div  x-data="{ open: false }" class="{{ $activeTab === 'customers' ? '' : 'hidden' }} mt-4 bg-white shadow rounded-lg p-6">
                <!-- Customer Filters with Add Client Button -->
                ...
                <div class="mt-4">
                        ...
                        <table class="min-w-full divide-y divide-gray-200" >
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($customers as $customer)
                                    <tr class="hover:bg-gray-50 transition-colors duration-150" wire:key="customer-{{ $customer->id }}">
                                        <td class="px-3 py-2">
                 ...
                 <div x-show="open"
                    ...
                    x-on:client-added.window="open = false"
                    x-cloak>
                    <div @click.away="open = false" class="bg-white rounded-lg shadow-xl max-w-md w-full p-6 relative">
                        ...
                        <form wire:submit="createClient">
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input type="text" id="client_name" wire:model="newClient.client_name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                                    @error('newClient.client_name') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                </div>
                                
                                <div>
                                    <label for="client_email" class="block text-sm font-medium text-gray-700">Email</label>
                                    <input type="email" id="client_email" wire:model="newClient.client_email" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                                    @error('newClient.client_email') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                </div>
                                
                                <div>
                                    <label for="mobile_number" class="block text-sm font-medium text-gray-700">Mobile Number</label>
                                    <input type="text" id="mobile_number" wire:model="newClient.mobile_number" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    @error('newClient.mobile_number') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                </div>
                                
                                <div>
                                    <label for="address" class="block text-sm font-medium text-gray-700">Address</label>
                                    <textarea id="address" wire:model="newClient.address" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" rows="2"></textarea>
                                    @error('newClient.address') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                </div>
                            </div>
                            
                            <div class="mt-5 flex justify-end space-x-3">
                                <button type="button" @click="open = false" class="px-4 py-2 bg-gray-100 text-gray-700 text-sm font-medium rounded-md hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-500">
                                    Cancel
                                </button>
                                <button type="submit" class="px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500">
            ...
            <div class="{{ $activeTab === 'logs' ? '' : 'hidden' }} mt-4">
                <!-- Keep the existing email logs tab content unchanged -->
                <!-- Filters -->
